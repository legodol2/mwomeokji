...
장보기 화면에서 **‘장 봤어요’**를 누르면 이번에 쓰고 남는 양이 저장됩니다. 다음에 식단을 짤 때 그 재료부터 쓰고, 모자란 만큼만 장보기 목록에 올립니다. 계란 10구를 사서 2개만 썼다면 8개가 남고, 다음에 5개가 필요하면 사지 않고 집에 있는 걸로 채웁니다.

- 보관 기간(`ING[id].s`)이 지난 재고는 자동으로 계산에서 빠집니다 — 상추 4일, 계란 21일처럼 품목마다 다릅니다
- 설정 > **집에 남은 재료**에서 목록과 남은 보관일을 보고, 하나씩 지우거나 전부 비울 수 있습니다

## 가격에 대해

가격은 **중간 가격대(일반) 제품** 기준입니다. 같은 품목이라도 매대에는 행사상품부터 프리미엄까지 있어서, 설정 > 장바구니 가격대에서 알뜰(−15%)·일반·좋은 것(+25%) 중에 고를 수 있고 기본은 일반입니다. 마트별 가격 배율(`src/data.js`의 `MART`)은 특정 마트를 조사한 값이 아니라 매장 유형에 따른 **가정**입니다. 또 마트 실시간 가격 API는 공개된 것이 없어서, **국내 대형마트·전통시장의 통상 판매 시세를 넣은 내장 표**에 위 계수를 곱한 **추정치**입니다. 실제 매대 가격은 행사·산지·시기에 따라 20%씩 움직이니 총액은 여유 있게 잡으세요. 시세를 고치려면 `src/data.js`의 `ING`에서 `p`(파는 단위 가격)와 `pq`(단위 수량)만 손보면 됩니다.
